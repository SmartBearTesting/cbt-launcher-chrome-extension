    ...
    options = {
        ops:{
            "input":[
                ...
                {"name":"PopupTitleLink","label":"Title link","type":"text"},
            ...
            //    {"label":"Popup config","type":"title","tag":"h3", parent:'default'},
                ...
                {"name":"PopupDateformat","label":"Date format","type":"text"},
                {"name":"PopupReverseOrder","label":"Reverse order","type":"checkbox"},
                {"name":"DifferentiateApproved","label":"Differentiate approved","type":"checkbox"},
...
$(document).ready(function(){
    ...
    $('.json-viewer').on('click',function(e){
        ...
        if (backgroundPage.kmj.items && backgroundPage.kmj.items.length){
            item = backgroundPage.kmj.items[0];
            queryString = encodeURIComponent( JSON.stringify( item ) );
            window.open('http://jsonlint.com/?json='+ queryString);
        } else {
            alert("Sorry, no stash item found, are you logged in?");
        }
    });
    ...
    $('#PopupReverseOrder').on('click',function(){
